 ...
 *         2011-8-23
 */
public class CopyOfHttpDownloader {
	...
	public int download(Context context, String urlStr, String path, String fileName) {
		...
		CopyOfFileUtils fileUtils = new CopyOfFileUtils();
		if (fileUtils.isFileExist(path + fileName)) {
			((WorkDetailActivity) context).sendMsg(2, 0);
			return 1;
		} else {
			try {
	...
	public InputStream getInputStreamFromUrl(Context context, String urlStr) throws IOException {
		...
		((WorkDetailActivity) context).sendMsg(0, conn.getContentLength());
